...
function addToolbarButton(panel) {
    ...
    btn.addEventListener('command', function() {
        ...
        if (!USER_TOKEN) {
            ...
            var userGET = IAPI.requestURL("UserName_Get");
...
// TODO: change return to JSON return
function getModule() {
    require("page-worker").Page({
        contentURL: IAPI.requestURL("Modules", { Duration : 1, IncludeAllInfo : true }),
        contentScript: "self.postMessage(document.getElementsByTagName('pre')[0].innerHTML);",
        contentScriptWhen: "ready",
        onMessage: function(message) {
            console.log(message);
...
function populate_UserName() {
    ...
    //r.open("GET", IAPI.requestURL("Modules", { Duration : 1, IncludeAllInfo : true }), true);
