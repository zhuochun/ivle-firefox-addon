...
const BadgedWidget = Trait.compose({
  ...
  _makeContentURL: function _makeContentURL(imageURL, badge) {
    let str = 
      ...
      '<img src="' + imageURL + '">' +
      '<span style="font-size: 0.6em; font-weight: bold; position: absolute; bottom: 0; left: 0; padding: 0 0.2em; margin: 0;">' +
      (badge.text || '') + '</span>' +
      '<span style="font-size: 0.6em; background-color: ' + (badge.color || '') + ';' +
      'position: absolute; bottom: 0; left: 0; opacity: ' + (badge.opacity || '') + '; padding: 0 0.2em; margin: 0; -moz-border-radius: 0.4em;">' +
      (badge.text || '') + '</span>' +
      '</div>' +
      '</body></html>';
    return str;
